...
if (isset($_POST['submit'])) {
    ...
    if (!empty($token)) {
        ...
        $check_token_seeker = mysqli_query($conn, "SELECT verify_token FROM seeker_account WHERE verify_token='$token' LIMIT 1");
        ...
        } elseif (mysqli_num_rows($check_token_seeker) > 0) {
            
            if ($new_password == $confirmpass) {
                
                $encrypted_new_pass = password_hash($new_password, PASSWORD_DEFAULT);

                $update_password = mysqli_query($conn, "UPDATE seeker_account SET Password='$encrypted_new_pass' WHERE verify_token='$token' LIMIT 1");

                if ($update_password) {
                    
                    $new_token = md5(rand());
                    $update_to_new_token= mysqli_query($conn, "UPDATE seeker_account SET verify_token='$new_token' WHERE verify_token='$token' LIMIT 1");
